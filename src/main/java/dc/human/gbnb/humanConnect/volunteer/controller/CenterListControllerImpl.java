...
public class CenterListControllerImpl implements CenterListController {
    ...
    public ModelAndView viewCenterList(HttpServletRequest request, HttpServletResponse response,
        ...
        int pageNo;
        try {
            pageNo = Integer.parseInt(pageNoStr); // 기본값 및 예외 처리 추가
        } catch (NumberFormatException e) {
            pageNo = 1; // 기본 페이지 번호
        }

        StringBuilder urlBuilder = new StringBuilder("http://apis.data.go.kr/1543061/animalShelterSrvc/shelterInfo");
        urlBuilder.append("?" + URLEncoder.encode("serviceKey", "UTF-8") + "=FLy6xC7nTacvM663y4Qv%2FbhAlCm9OwG9K1xBr793iGIAQu24Z625CHW0C5c5dhZL%2Bna1qTW0TIBM8GhIzYzxLw%3D%3D");
        ...
        Set<Map<String, String>> careInfoSet = new HashSet<>();
        Pattern careNmPattern = Pattern.compile("<careNm>(.*?)</careNm>");
        Pattern careAddrPattern = Pattern.compile("<careAddr>(.*?)</careAddr>");
        Pattern careTelPattern = Pattern.compile("<careTel>(.*?)</careTel>");
        Matcher careNmFinder = careNmPattern.matcher(sb);
        Matcher careAddrFinder = careAddrPattern.matcher(sb);
        Matcher careTelFinder = careTelPattern.matcher(sb);

        while (careNmFinder.find()) {
            String careNmContent = careNmFinder.group(1);
            if (careAddrFinder.find() && careTelFinder.find()) {
                ...
                careInfoSet.add(careInfoMap);
            }
        }

        List<Map<String, String>> careInfo = new ArrayList<>(careInfoSet);

        int totalRecords = careInfo.size();
        int totalPages = (int) Math.ceil((double) totalRecords / numOfRows);
        ...
        // 페이지 번호가 총 페이지 수보다 클 경우 총 페이지 수로 설정
        if (pageNo > totalPages) {
            pageNo = totalPages;
        }

        int startIdx = (pageNo - 1) * numOfRows;
        int endIdx = Math.min(startIdx + numOfRows, totalRecords);

        List<Map<String, String>> pagedCareInfo = careInfo.subList(startIdx, endIdx);

        ModelAndView mav = new ModelAndView("centerList");
        mav.addObject("careInfo", pagedCareInfo);
        mav.addObject("totalRecords", totalRecords);
        mav.addObject("numOfRows", numOfRows);
...
//   }
    ...
    @Override
    @RequestMapping("/volCLDownload")
    public void volCLDownload(@RequestParam("imageFileName") String imageFileName,
